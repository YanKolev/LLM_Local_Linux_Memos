...
- Memory is managed by kernel. hardware is shared by all processes on the system via virtual addressing. Virtual addressing allows many processes to access same memory without conflict or crashes. it is done by using areas of hard dist to be used in place of physical ram. memory is divided by blocks of equally sizezd unites and can be addressed like any other resource.  access memory can be located elsewhere- or a different computer/virtual devie. Kernel space is where code for the kernel is stored and executed. This is generally in a “protected” range of memory addresses and remains isolated from other processes with lower privileges. User space, on the other hand, is available to users and programs. They communicate with the Kernel through “system call” APIs that act as intermediaries between regular programs and the Kernel.

- can be checked via the **free** command - provides snapshot of used memory. 
```
# monitor usage over time, -s (how ofen to update) + number of seconds
free -s 10 
free -s 5 

# for megabites version 
free -ms 5

# for gigabytes version 
free -gs 5
```
- swap memory: also known as a virtual memory. this is the space on the hard disk that is used like physical memory when the amount of physical memory becomes low. 
...
- **Log Files**: some processes can log their own data to these files, other processes rely on separate process (a daemon) to handle these log data files. 

- different variations: syslog, klog. Recent distro, logging daemon is called journald, mainly for text output. to use it **journalctl**.  Log files are almost always placed in the **/var/log** directory. 

![](images/Essentials/logfiles.png)

- **cat / less** can be used to check those logs. Log files are rotated, older log files are renamed and replaced with newer log files. The file names that appear in the table above may have a numeric or date suffix added to them: for example, secure.0 or secure-20181103

- rorating file occurs on regularly-scheduled basis. once rotated system stops writing to the log file and adds a suffix to it. then a new file with the original name is created and the logging continue into the newfile. date is often the suffix. 
- for reading file content we can also use **file**, ex file /var/log/btmp (it will show that that the info is "data" meaning binary).
- to view binary data: **lastb /last**.

--- 
- **Kernel Messages**: /var/log/dmesg contains the kernel messages taht were produced during startup. Kernel does not have its own log file. it can be configured: y modifying either the /etc/syslog.conf file or the /etc/rsyslog.conf file. In addition, the dmesg command can be used to view the kernel ring buffer, which holds a large number of messages that are generated by the kernel.‌⁠​​⁠​ 

- **dmesg**- can produce up to 512kb text. it needs to be filtered with less or grep. grep is recommended. 
- to troubleshoot USB device, and to ignore case:
```
dmesg | grep -i usb
```
...
![](images/Essentials/FHS1.png)
---

![](images/Essentials/FHS2.png)
---
![](images/Essentials/FHS3.png)
...
# to bring into forground
fg %1

# to pause process /regain control of the terminal ctrl+Z

# to have the process continue executing in the abckground
bg %1

# to kill a job > it will lead to termination of the process.
kill %1 
kill %2
kill %3

# to stop all ping we can use 
#killall ping
```

- top, to view processes in descending order of percentage of CPU usage.
- to operate with top we can press K, to kill a process but we need to eb carefulwith PID. for more info **man kill**
- to kill remaining processes > PID > send value 9 instead of default + enter
- **kill signal 9 or SIGKILL- forceful signal that can not be ignored unlike default 15**.
- to exit: q
- **sleep**: used to pause a program / shell script for a specific period of time. 
- again it needs to be ended with ampersand at end to run it without losing terminal control. 
- to kill sleep command
```
# will only display the processes runnin in the current shell
ps
kill PID
jobs
```
- **pkill**: pkill -15 sleep
- **ps**:  command can be used to view processes. with flag -e: all processes are displayed.
```
#with flag -o to specify which columns to output
ps -o

#example of requested columns
ps -o pid,tty,time,%cpu,cmd

# option to sort which option to specify which colums to sort by. goes by ascending order. to sort the output of ps by %mem
...
- viewing system logs, there are two daemons that handle log messages: **syslog** daemon and **klogd** daemon. (klodg-kernel logs) > then it sends log information to syslogd daemon. 

- messages were generated by the kernel at boot time are stored in the /var/log/dmesg. 
- **dmesg**- allows viewing current kernel messages, as well as providing control of those messages will apear in a terminal. 

- main log file written to by syslog is in **/var/log/ messages**
...
- test 2 - Score: 78% | 39 correct out of 50 
