...
- Package Management.  Debian uses **dpkg**, but it can be tricky for novice Linux users, so Advanced Package Tool **apt-get** , a front-end program to the **dpkg** tool makes management of packages easier.


- Before installing packages is a good practice to update them via the **apt-get update** command. 

- To search for keywords within packages we can use **apt-cache search**. 
```
...
- after discovery you can install with: 
```
sudo apt-get intall [package]
```

- Updating package: **apt-get install** can also update package if that package is installed and a newer version is available. IF the package is not on the system it will be installed.

- Updating all packages of the system should be done in two steps: 
  1. update cache of all packages available.
   ```
    apt-get update
   ```
  2. execute apt-get upgrade
   ```
    apt-get upgrade
   ```

- Removing package: **apt-get** is able to remove or purge package. Purging deletes all package files, while remove deletes all but the configuration files of the package. 
```
apt-get remove [package]
apt-get purge [package]
```
- with purge the package is completely removed from the system.
...
- **passwd** isthe command to update user's password. Users can only change their password whereare the root user can update the password for any user. 

```
passwd [OPTIONS] [USER]

# if the user wants to viewthe status information of their password they can use the -S option 
...
![](images/Unhatched/passwords.png)

- if we want to change the password of the root password > su root + password > passwd sysadmin > enter new password > exit. 
...
- Redirection. Adding content to linux files can be done in many ways. Via text editors or with direct **input/output (I/O)**. The input or output of a command is redirected from its default destination to a different location. I/O redirection is like a series of train tracks, where a switch can be enabled to direct the output of a command on a different track so it goes somewhere else in the shell. 
...
- This is useful if you need to copy content from an important file to another file in order to edit the contents without modifying the original file. However, what if you want to add a comment or note to a file? To do this, you can use the **echo** command.

- NB! if we use **echo**, and then output to STDOUT with >, it will replace the original content. 
  ```
  > will overwrite any contents of an existin file 
  >> to append instaed of overwrite
  ```
- NB! to redirect information permissions are needed. 
...
- Text Editors. most Famous- **vi** editor. 
...
There are 3 modes in vi: Command mode, inster mode and ex mode. 

- initially the program starts in command mode. it is used to type command such as: move around document, manipulate text, access the other two modes. to return to command mode **Esc** key. 

- when text has ben added into a document > Esc press.

- Movement command in vi have 2 aspects, a motion and a optional numner prefix- which indicates how many times to repeat that motion. 
...
- Commands: 
![](images/Unhatched/vimotions1.png)

- Those motions can be prefixt with a number to indicate how many times to perform the movement. 
```
5h - move the cursor 5 characters to the left
3w - move the cursor 3 words to the right

# To move specifi line number, type line number followrd by G character

5G- fifth line of the file
1G / gg - to go to first line of the file
G- last line of the file.
CTRL+G - which line the cursor is currently on
```

- Command mode Actions: vi does not have: copy, cut paste. vi uses 3 commands: 

![](images/Unhatched/vimotions2.png)

- motions always start at present curson location. There are 2 general formats for action commands:
```
action [count] motion
[count] action motion
```

- Delete- removes the indicated text from the page, saves it into the buffer ( buffer being the equivalet of clipboard for windows/mac).

- Change- simiral to delete, text is removed and saved into the buffer, the program is switched to insert mode to allow immediate changes to the text. 

![](images/Unhatched/vimotions3.png)


- Yank- places content into the buffer without deleting it.

- Put- places the text saved in the buffer either before or after the cursor positon. Put does not use motions like previous commands. 
  
![](images/Unhatched/vimotions4.png)

- Searching in vi. instead of ctrl+f vi uses search. it is more powerful > supports bot literal text patterns and regular expressions. 

- to seach forward from the current postion of the cursor, use **/** to start searc > type a search term > press Enter to beging search. 

- to jump to next match using the same pattern use **n** key. 
- to jump to previus pattern use **N** key. 
- the search will automatically wrap around to the other side of the document.
- to start searching backwards from the cursor positon, start typing **?**, then pattern to search and press Enter. 

- Insert Mode. It is used to add text to the document. 

![](images/Unhatched/vimotions5.png)


- Ex Mode. t is possible to view or change settings, as well as carry out file-related commands like opening, saving or aborting changes to a file. In order to get to the ex mode, type a **:** character in command mode. 
![](images/Unhatched/vimotions6.png)


- if an exclamation mark, !, is added to a command, it then attempts to force the operation. For example, imagine you make changes to a file in the vi editor and then try to quit with :q, only to discover that the command fails. The vi editor doesn't want to quit without saving the changes you made to a file, but you can force it to quit with the ex command :q!.
